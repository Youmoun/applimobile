...
export default function App(){
  ...
  const [showFilters, setShowFilters] = useState(true)   // << repliable
  ...
  const [myProvider, setMyProvider] = useState<Provider | null>(null) // pour masquer "Je suis prestataire"
  ...
  // charger mon profil si connecté (pour bloquer la création multiple)
  useEffect(() => {
    const loadMine = async () => {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user?.id) { setMyProvider(null); return }
      const { data } = await supabase
        .from('providers')
        .select('*, services(*), ratings(stars)')
        .eq('user_id', user.id)
        .maybeSingle()
      setMyProvider((data || null) as any)
    }
    loadMine()
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <main className="max-w-6xl mx-auto px-4">
        {/* Section d’accueil masquée une fois la recherche démarrée */}
        {!started && (
          <section className="py-10 sm:py-16 grid lg:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <button className="btn" onClick={()=> { setStarted(true); setShowFilters(true) }}>🔎 Trouver un prestataire</button>
                <button
                  className="btn-secondary px-4 py-2 rounded-xl"
                  onClick={()=>{
                    if (myProvider) return alert('Vous avez déjà un profil prestataire. Utilisez “Mon profil” pour le modifier.')
                    setSignupOpen(true)
                  }}
                >
                  👤 Je suis prestataire
            ...
            <div className="card p-4">
              ...
              <div className="mt-3">
                <button className="btn w-full" onClick={()=> { setStarted(true); setShowFilters(false) }}>Lancer la recherche</button>
              </div>
            </div>
          </section>
        )}

        {/* Bandeau au-dessus des résultats + bouton pour ré-ouvrir les filtres */}
        {started && (
          <div className="sticky top-[56px] z-30 bg-white/90 backdrop-blur border-b mb-3 py-2 px-3 flex items-center justify-between">
            <div className="text-sm text-gray-600">
              {filtered.length} résultat{filtered.length>1 ? 's' : ''}{queryCity ? ` à ${queryCity}` : ''}{queryCategory ? ` • ${queryCategory}` : ''}
            </div>
            <div className="flex items-center gap-2">
              <button className="btn-secondary px-3 py-1 rounded-xl" onClick={() => setShowFilters(v => !v)}>
                {showFilters ? 'Masquer la recherche' : 'Modifier la recherche'}
              </button>
            </div>
          </div>
        )}

        {/* Zone filtres repliable + résultats */}
        {started && (

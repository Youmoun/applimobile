...
export function ProviderSignup({ onCreated }:{ onCreated: () => void }){
  ...
  async function handleSubmit(e:React.FormEvent){
    ...
    if(!first_name || !last_name || !department || !city || !phone) return
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid sm:grid-cols-2 gap-4">
        <div><label className="block text-sm mb-1">Prénom</label><input className="input" value={first_name} onChange={e=>setFirst(e.target.value)} required /></div>
        ...
        <div>
          ...
          <select className="input" value={department} onChange={e=>{ setDepartment(e.target.value); setCity('') }} required>
        ...
        <div>
          ...
          <select className="input" value={city} onChange={e=>setCity(e.target.value)} required disabled={!department}>
            <option value="">Sélectionner...</option>
            {(DEPTS[department] || []).map(v=> <option key={v} value={v}>{v}</option>)}
          </select>
        </div>
        <div><label className="block text-sm mb-1">Téléphone</label><input className="input" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="+33 6 00 00 00 00" required /></div>
        <div>
          <label className="block text-sm mb-1">Photo (depuis la galerie)</label>
          <input type="file" accept="image/*" onChange={(e)=> setFile(e.target.files?.[0] ?? null)} className="input" />
          <p className="text-xs text-gray-500 mt-1">Ou collez une URL ci-dessous (optionnel)</p>
          <input className="input mt-1" value={photo_url} onChange={e=>setPhoto(e.target.value)} placeholder="https://..." />
        </div>
        <div className="sm:col-span-2"><label className="block text-sm mb-1">À propos</label><textarea className="input" value={about} onChange={e=>setAbout(e.target.value)} placeholder="Décrivez votre activité en quelques lignes" /></div>
      </div>
      ...
      <button type="submit" className="btn w-full" disabled={saving}>{saving ? 'Création...' : 'Créer mon profil prestataire'}</button>
    </form>
  )
}

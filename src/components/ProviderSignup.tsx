    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="sm:col-span-2">
          <button type="button" className="btn-secondary px-3 py-2 rounded-xl" onClick={useMyPosition}>📍 Utiliser ma position</button>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {services.map((s, i)=>(
            <div key={i} className="grid grid-cols-12 gap-2 items-center">
              ...
              <div className="col-span-1 text-right">{services.length>1 && <button type="button" className="btn-secondary px-3 py-2 rounded-xl" onClick={()=>setServices(prev=>prev.filter((_,idx)=>idx!==i))}>✕</button>}</div>
            </div>
          ))}
        </div>
      </div>

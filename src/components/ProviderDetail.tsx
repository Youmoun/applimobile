...
export function ProviderDetail({ provider, onClose, refresh }:{ provider: Provider, onClose: () => void, refresh: () => void }) {
  ...
  async function sendRating() {
    ...
    // upsert: un seul avis par couple (provider_id, user_id)
    const { error } = await supabase
      .from('ratings')
      .upsert(payload, { onConflict: 'provider_id,user_id' })
    if (error) {
      alert('Erreur enregistrement de la note : ' + error.message)
      return
    }
    await refresh()
  }
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4 z-50">
      <div className="card max-w-2xl w-full p-6 bg-white relative">
        ...
        <div className="mt-6 border rounded-2xl p-3 space-y-3">
          ...
          <p className="text-xs text-gray-500">Un seul avis par utilisateur et par prestataire. Relancer la note mettra simplement à jour la vôtre.</p>
        </div>
      </div>
    </div>
  )
